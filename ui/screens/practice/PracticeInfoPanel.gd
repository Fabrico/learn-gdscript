...
@onready var title_label := find_child("Title") as Label
@onready var _status_icon := find_child("StatusIcon") as TextureRect
...
func _ready() -> void:
	_list_button.connect("pressed", Callable(self, "emit_signal").bind("list_requested"))
