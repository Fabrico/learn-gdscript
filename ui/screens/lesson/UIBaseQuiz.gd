...
func setup(quiz: Quiz) -> void:
	...
	_content.text = TextUtils.bbcode_add_code_color(tr(_quiz.content_bbcode.replace("\r\n", "\n")))

	_explanation.visible = not _quiz.explanation_bbcode.is_empty()
	# FIXME: Some weird Windows issue, replace before translating so matching works.
	_explanation.text = TextUtils.bbcode_add_code_color(tr(_quiz.explanation_bbcode.replace("\r\n", "\n")))
