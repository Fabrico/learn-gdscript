...
func _update_theme() -> void:
	...
	if tab_style is StyleBoxFlat:
		tab_style.bg_color = get_color("base_color", "Editor").lerp(
...
func search(query: String) -> void:
	...
	for index in range(start_index, block_count):
		...
		var text_edit_search_result: PackedInt32Array = block.search(query, start_line, start_column)
		if not text_edit_search_result.is_empty():
