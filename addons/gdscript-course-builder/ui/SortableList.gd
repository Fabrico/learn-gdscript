...
func _draw_overlay() -> void:
	...
	if _active_insert_area >= 0 and _active_insert_area < _insert_item_areas.size():
		...
		_overlay_layer.draw_colored_polygon(circle_points, insert_area_color, PackedVector2Array(), null, null, true)
...
func _get_drag_data(position: Vector2):
	...
	for child_node in _item_list.get_children():
		...
		if child_node.has_method("get_drag_target_rect"):
			...
			drag_rect.position += child_node.global_position
		else:
...
func _can_drop_data(position: Vector2, drag_data) -> bool:
	if not drag_enabled:
		return false
	...
	for child_node in _item_list.get_children():
		...
		if child_node.has_method("get_drop_target_rect"):
			...
			drop_rect.position += child_node.global_position
		else:
			drop_rect = child_node.get_global_rect()

		drop_rect.position -= global_position
...
func _drop_data(position: Vector2, drag_data) -> void:
	if not drag_enabled:
		return
