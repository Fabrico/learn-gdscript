...
func _ready() -> void:
	...
	_remove_item_button.connect("pressed", Callable(self, "_on_remove_item_requested"))
...
func _update_theme() -> void:
	...
	if _selected_style is StyleBoxFlat:
		_selected_style.bg_color = get_color("base_color", "Editor").lerp(
			get_color("contrast_color_1", "Editor"), 0.35
		)

	_drag_preview_style = _normal_style.duplicate()
	if _drag_preview_style is StyleBoxFlat:
		_drag_preview_style.bg_color = get_color("base_color", "Editor").lerp(
			get_color("dark_color_1", "Editor"), 0.35
		)

	_background_panel.add_theme_stylebox_override("panel", _normal_style)

	_lesson_name_label.add_theme_font_override("font", get_font("title", "EditorFonts"))
	_lesson_path_label.add_theme_color_override("font_color", get_color("disabled_font_color", "Editor"))
...
func set_lesson(lesson: Lesson) -> void:
	...
	_lesson_path_label.text = _edited_lesson.resource_path.trim_prefix(_base_path)
	_lesson_path_label.tooltip_text = _edited_lesson.resource_path
...
func set_selected(value: bool) -> void:
	...
	if _is_selected:
		_background_panel.add_theme_stylebox_override("panel", _selected_style)
	else:
		_background_panel.add_theme_stylebox_override("panel", _normal_style)
